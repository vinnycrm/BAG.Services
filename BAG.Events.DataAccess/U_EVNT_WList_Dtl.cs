...
public class U_EVNT_WList_DtlDAL
{
    ...
    public EventSummary Select_EVNT_WList(string wishlist_Id, string UserId)
	{
		// SqlConnection that will be used to execute the sql commands
		SqlConnection connection = null;
        SqlParameter[] aParms = new SqlParameter[] { new SqlParameter(PARAM_WList_Id, wishlist_Id), new SqlParameter(PARAM_Id, UserId) };
        List<ItemsList> olist = new List<ItemsList>();
        List<Invites> olist2 = new List<Invites>();
		...
		try
		{
			...
			SqlDataReader reader = SqlHelper.ExecuteReader(connection, CommandType.Text, SQL_SELECT_U_EVNT_WList_Dtl, aParms);
            ...
            osummary.ItemList = olist.ToArray();
            osummary.InvitedMembers = olist2.ToArray();
			// close Reader
			reader.Close();
			return osummary;
		}
